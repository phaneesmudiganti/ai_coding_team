...
class GenerationOps:
    ...
    def write_architecture(project_name: str, output_dir: str, file_name: str, content: str):
        ...
        logger.info("architecture written")
        return f"Architecture file created: {arch_path}"

    @staticmethod
    def write_code(module_name: str, project_name: str, output_dir: str, code_stub: str = "", file_extension: str = None):
        os.makedirs(output_dir, exist_ok=True)
         # 1. Determine file extension priority:
        #    - if explicitly provided via tool arguments -> use it
        #    - else infer based on module_name
        if file_extension:
            ext = file_extension
        else:
            # Intelligent fallback logic
            if "frontend" in module_name.lower():
                ext = "tsx"
            elif "react" in module_name.lower():
                ext = "tsx"
            elif "ui" in module_name.lower():
                ext = "jsx"
            else:
                ext = "py"

        file_name = f"{project_name}_{module_name}.{ext}"
